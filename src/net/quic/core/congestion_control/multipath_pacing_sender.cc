...
MultipathPacingSender::MultipathPacingSender()
    : sender_(nullptr), max_pacing_rate_(QuicBandwidth::Zero()), burst_tokens_(
        kInitialUnpacedBurst), last_delayed_packet_sent_time_(QuicTime::Zero()), ideal_next_packet_send_time_(
        QuicTime::Zero()), was_last_send_delayed_(false), consecutive_making_up_time_(0) {
}

MultipathPacingSender::~MultipathPacingSender() {
}

void MultipathPacingSender::set_sender(
    MultipathSendAlgorithmInterface* sender) {
  DCHECK(sender != nullptr);
  sender_ = sender;
}

void MultipathPacingSender::OnCongestionEvent(
    const QuicSubflowDescriptor& descriptor, bool rtt_updated,
    QuicByteCount bytes_in_flight, QuicTime event_time,
  ...
  sender_->OnCongestionEvent(descriptor, rtt_updated, bytes_in_flight,
      event_time, acked_packets, lost_packets);
}

bool MultipathPacingSender::OnPacketSent(
    const QuicSubflowDescriptor& descriptor, QuicTime sent_time,
    QuicByteCount bytes_in_flight, QuicPacketNumber packet_number,
    QuicByteCount bytes, HasRetransmittableData has_retransmittable_data) {
  DCHECK(sender_ != nullptr);
  const bool in_flight = sender_->OnPacketSent(descriptor, sent_time,
      bytes_in_flight, packet_number, bytes, has_retransmittable_data);
  if (has_retransmittable_data != HAS_RETRANSMITTABLE_DATA) {
    return in_flight;
  }
  // If in recovery, the connection is not coming out of quiescence.
  if (bytes_in_flight == 0 && !sender_->InRecovery(descriptor)) {
    ...
    burst_tokens_ = std::min(kInitialUnpacedBurst,
        static_cast<uint32_t>(sender_->GetCongestionWindow(descriptor)
            / kDefaultTCPMSS));
  }
  if (burst_tokens_ > 0) {
    --burst_tokens_;
  ...
  QuicTime::Delta delay =
      PacingRate(descriptor, bytes_in_flight + bytes).TransferTime(bytes);
  ...
  if (was_last_send_delayed_) {
    ...
    const bool making_up_for_lost_time = ideal_next_packet_send_time_
        <= sent_time;
    ...
    if (making_up_for_lost_time && !application_limited &&
        (kMaxConsecutiveMakingUpTime == 0 ||
            consecutive_making_up_time_ < kMaxConsecutiveMakingUpTime)) {
      ++consecutive_making_up_time_;
      last_delayed_packet_sent_time_ = sent_time;
    } else {
      consecutive_making_up_time_ = 0;
      was_last_send_delayed_ = false;
      last_delayed_packet_sent_time_ = QuicTime::Zero();
    }
  } else {
    consecutive_making_up_time_ = 0;
    ideal_next_packet_send_time_ = std::max(
        ideal_next_packet_send_time_ + delay, sent_time + delay);
  }
  return in_flight;
}

QuicTime::Delta MultipathPacingSender::TimeUntilSend(
    const QuicSubflowDescriptor& descriptor, QuicTime now,
    QuicByteCount bytes_in_flight) {
  DCHECK(sender_ != nullptr);
  QuicTime::Delta time_until_send = sender_->TimeUntilSend(descriptor, now,
      bytes_in_flight);
...
QuicBandwidth MultipathPacingSender::PacingRate(
    const QuicSubflowDescriptor& descriptor,
    QuicByteCount bytes_in_flight) const {
  DCHECK(sender_ != nullptr);
  //QuicBandwidth maxPacingRate = sender_->GetMaxBandwidth(descriptor, bytes_in_flight);
  if (!max_pacing_rate_.IsZero()) {
    return QuicBandwidth::FromBitsPerSecond(
        std::min(max_pacing_rate_.ToBitsPerSecond(),
            sender_->PacingRate(descriptor, bytes_in_flight).ToBitsPerSecond()));
  }
  return sender_->PacingRate(descriptor, bytes_in_flight);
}
